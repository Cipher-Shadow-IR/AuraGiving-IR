    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        >
          ...
          {/* Main Banner */}
          <div className="relative rounded-xl overflow-hidden border border-white/[0.08] bg-[#0c0d12] h-72 sm:h-80">
            <img
              src={campaign.image}
              alt={campaign.title}
              className="w-full h-full object-cover"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#050508] via-transparent to-transparent opacity-85" />

            <div className="absolute top-3.5 left-3.5">
              <span className="px-2.5 py-0.5 rounded text-[11px] font-medium bg-[#000000]/90 border border-white/[0.08] text-slate-200">
          ...
          {/* Organizer Card */}
          <div className="p-4 rounded-xl gradient-border flex items-center justify-between gap-4">
            <div className="space-y-0.5">
              <p className="text-[10px] uppercase font-semibold text-slate-500 tracking-wider font-mono">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-xs text-slate-400 hover:text-emerald-400 inline-flex items-center gap-1 font-mono"
            >
              <span>Explorer</span>
          ...
          {/* Mission Description */}
          <div className="p-6 rounded-xl gradient-border space-y-3">
          ...
          {/* Backer Leaderboard */}
          <div className="p-6 rounded-xl gradient-border space-y-4">
            ...
            {sortedDonators.length > 0 ? (
              ...
              <div className="text-center py-6 text-xs text-slate-500 border border-dashed border-white/[0.06] rounded-lg font-mono">
        ...
        <motion.div 
          ...
          className="lg:col-span-5 sticky top-20 space-y-4"
        >
          
          {/* Key Metrics Grid */}
          <div className="grid grid-cols-3 gap-2.5">
            <div className="p-3.5 rounded-xl gradient-border text-left">
              <p className="text-[11px] font-mono text-slate-500 uppercase tracking-wider">Days Left</p>
              <p className="font-mono font-bold text-lg text-white mt-0.5">{isExpired ? "0" : remainingDays}</p>
            </div>

            <div className="p-3.5 rounded-xl gradient-border text-left">
              <p className="text-[11px] font-mono text-slate-500 uppercase tracking-wider">Raised</p>
              <p className="font-mono font-bold text-lg text-emerald-400 mt-0.5">{campaign.amountCollected} <span className="text-xs font-normal text-slate-400">ETH</span></p>
            </div>

            <div className="p-3.5 rounded-xl gradient-border text-left">
              <p className="text-[11px] font-mono text-slate-500 uppercase tracking-wider">Backers</p>
              <p className="font-mono font-bold text-lg text-white mt-0.5">{donators.length}</p>
            </div>
          </div>

          {/* Donation Station */}
          <div className="p-5 rounded-xl gradient-border space-y-4 shadow-sm">
            ...
            <form onSubmit={handleDonate} className="space-y-4 pt-1">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="btn-sheen w-full py-2.5 rounded-lg font-semibold text-slate-950 bg-emerald-400 hover:bg-emerald-300 transition-all flex items-center justify-center gap-2 text-xs active:scale-95 disabled:opacity-50"
              >
                {isLoading ? (
                  <>
